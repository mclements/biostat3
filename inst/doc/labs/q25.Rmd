...
```{r ex_25_coxph,  message=FALSE, warning=FALSE, results='hide'}
```

## (a) ## 
How many individuals are in the study?

```{r  n_ind,  message=FALSE, warning=FALSE, results='hide'}
```

## (b) ## 
How many experience the event?

```{r  n_event,  message=FALSE, warning=FALSE, results='hide'}
```
...
```{r gen_ncc,  message=FALSE, warning=FALSE, results='hide'}
```
...
```{r clogit,  message=FALSE, warning=FALSE, results='hide'}
```

## (d) ## 
How many unique individuals are in our study?

```{r n_unique_ncc,  message=FALSE, warning=FALSE, results='hide'}
```
...
```{r compare_coh_ncc,  message=FALSE, warning=FALSE, results='hide'}
```
...
```{r output,  message=FALSE, warning=FALSE, results='hide', fig.show='hide'}
```
